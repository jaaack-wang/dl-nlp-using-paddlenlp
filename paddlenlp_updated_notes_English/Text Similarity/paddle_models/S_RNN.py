...
About: Simple RNN model for text matching using paddle. 
'''
...
class SimpleRNN(nn.Layer):
    ...
    def __init__(self,
        ...
        self.rnn = nn.SimpleRNN(
            embedding_dim, rnn_hidden_dim, n_layers, self.direction, dropout=dropout_rate)
        
        rnn_out_dim = rnn_hidden_dim * 2 if bidirectional is True else rnn_hidden_dim
        self.dense = nn.Linear(rnn_out_dim * 2, hidden_dim_out)
