...
def build_batches(dataset, 
    ...
    def pad(batch):
        if max_seq_len:
            max_len1 = max_len2 = max_seq_len
        else:
            max_len1 = max(len(b[0]) for b in batch)
            max_len2 = max(len(b[1]) for b in batch)
        ...
        for idx, bt in enumerate(batch):
            ...
            # ----- for text a -----
            text_a.append(_pad(bt[0], max_len1))
            
            # ----- for text b -----
            text_b.append(_pad(bt[1], max_len2))
            
            # ----- for the label -----
            label.append(bt[-1])
        
        text_a = np.asarray(text_a, dtype=dtype)
        text_b = np.asarray(text_b, dtype=dtype)
        label = np.asarray(label, dtype=dtype)
        
        if include_seq_len:
            return [text_a, text_b, text_a_len, text_b_len, label]
